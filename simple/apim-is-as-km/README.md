...
![GitHub Logo](https://apim.docs.wso2.com/en/4.2.0/assets/img/setup-and-install/is-as-km-dbs.png)
...
3. Switch to `apim-docker-compose/simple/apim-is-as-km` folder.

   ```
   cd apim-docker-compose/simple/apim-is-as-km
   ```
4. Download the APIM-4.0.0 binary distribution from the wso2official website and copy the downloaded zip archive (wso2am-4.0.0.zip) into the `apim-docker-compose/simple/apim-is-as-km/dockerfiles/apim directory. (If you need to try out the flow in a specific U2 update level, update the downloaded binary distribution into the required U2 update level and create the zip file) 
   > This Docker Compose configuration is set up to create a custom Docker image by utilizing a binary that has been copied into the /dockerfiles directory. This approach ensures compatibility with various operating systems, including Mac M1, as the resulting Docker image will be tailored to the specific runtime environment. For guidance on configuring this setup to utilize WSO2 official Docker images, please refer to <a name="Building APIM Custom Docker Image">advanced configuration</a> . 

5. Download the WSO2IS-5.11.0 binary distribution from the wso2official website and copy it into the `apim-docker-compose/simple/apim-is-as-km/dockerfiles/is-as-km directory
...
5. Add a DNS record mapping the hostnames and the loopback IP or instance IP.
      ...
      IS node can be accessed through the `is.am.wso2.com` hostname

   ```
   # /etc/hosts
   127.0.0.1       is.am.wso2.com api.am.wso2.com gw.am.wso2.com 
   ```
...
└── dockerfiles
    ...
    │   │   └── wso2is.notification.event.handlers-1.0.17.jar
    │   ├── webapps
